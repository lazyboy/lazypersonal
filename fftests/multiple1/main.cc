...
//
// The code base is heavily influenced (and reused) from ffmpegthumbnailer
// by Dirk Vanden Boer (code.google.com/p/ffmpegthumbnailer).
...
struct Thumbnailer {
  ...
  void generateMultiThumbnail(string filename, string thumb_file) {
    MovieDecoder* movie_decoder;
    movie_decoder = new MovieDecoder(filename);
    printf("After call movie decoder generate\n");

    vector<Frame> frames;
    for (int i = 0; i < 3; ++i) for (int j = 0; j < 4; ++j) {
      printf("Generating %d\n", i * 4 + j);
      VideoFrame vframe;
      movie_decoder->getVideoFrame(vframe, 20+4*(i*4+j)); // Seek to each second
      Frame f;
      f.init(vframe.w, vframe.h, vframe.linesize, vframe.frame_data);
      frames.push_back(f);
    }
    MultiFrame* multi_frame = new MultiFrame(4);
    multi_frame->init(3, 4, frames);

    int linesize = multi_frame->getLinesize();
    int w = multi_frame->getW();
    int h = multi_frame->getH();
    printf("Merged video frame %d X %d, linesize = %d\n", w, h, linesize);

    // Convert video frame to png?
    vector<uint8_t*> row_ptrs;
    for (int i = 0; i < h; ++i) {
      row_ptrs.push_back( &(multi_frame->data[i * linesize]));
    }
    PngWriter writer;
    writer.writePng(&(row_ptrs.front()), w, h, thumb_file);
    multi_frame->destroy();
...
int main(int argc, char** argv) {
  ...
  if (argc < 3) {
    printf("Usage: %s filename outputpng_name\n", argv[0]);
    return -1;
  }

  Thumbnailer* thumb;
  thumb->generateMultiThumbnail(argv[1], argv[2]);
  return 0;
}
